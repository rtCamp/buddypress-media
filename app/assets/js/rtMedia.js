...
jQuery( 'document' ).ready( function( $ ) {
	 ...
	 */
	$( '.rtmedia-container' ).on( 'click', '.rtm-delete-media', function( e ) {
		e.preventDefault();
			...
			$.ajax( {
				...
				success: function( response ) {
					...
					if ( 'rtmedia-media-deleted' === response.data.code ) {
						...
						$('#user-media span, #media-groups-li #media span').text(response.data.all_media_count);

						// Update the count on sub navigations (Photo, Video & Music)
						$('#rtmedia-nav-item-photo span').text(response.data.photos_count);
						$('#rtmedia-nav-item-music span').text(response.data.music_count);
						$('#rtmedia-nav-item-video span').text(response.data.videos_count);
					} else {
						// Show alert message
